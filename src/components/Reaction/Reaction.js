...
const Reaction = ({user_id}) => {
    ...
    const{ availableReactions, usersReactions } = useContext(StoreContext);

    const Reactions = usersReactions.filter(userReaction => userReaction.user_id.toString() === user_id.toString());
    console.log(Reactions);
    const addReactionHandler = (emoji, name) => {
      let  ReactionObj = {
        emoji,
        name,
        user_id
    ...
    return (<div className={styles.addReaction} tabIndex="1" >
