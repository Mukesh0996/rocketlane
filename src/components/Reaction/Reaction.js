...
const Reaction = ({user_id}) => {
    ...
    // filter the reactions for each user based on the user_id
    let currentUsersReactions = usersReactions.filter(userReaction => userReaction.user_id === user_id);
 
    // get the index of the existing emoji 
    let existingReactionIndex = (emoji)=> arr.findIndex(elem => elem.emoji === emoji && elem.user_id === user_id);

    //run the loop only if the user has reactions 
    if( currentUsersReactions.length > 0 ) {
            currentUsersReactions.forEach((d) => {
                const requiredReaction = availableReactions.find(reaction => reaction.id === d.reaction_id);
                    if(requiredReaction) {
                        arr.push({user_id, emoji: requiredReaction.emoji, count: 1, id: d.id});
                    }
              });
    };
    
    const addReactionHandler = (reaction_id, emoji) => {
      let  ReactionObj = { user_id, content_id: 1,reaction_id };
        if(existingReactionIndex(emoji) === -1) {
            // add reaction to the user if the emoji is not present.
            addReaction(ReactionObj);
        }   else {
            // get Id of the existing reaction to perform DELETE operation.
    ...
    return (    
        ...
        <div className={styles.addReaction} tabIndex="1" >
            ...
            <div className={styles.existing_emojis}>
                { arr.length > 0 && arr.map((a, index) => <div onClick={()=> addReactionHandler(a.id, a.emoji)} id={a.id} key={index} className={`${styles.emoji} ${styles.reacted}`}> <span className={styles.size}>{a.emoji}</span> <span>.</span> <span>{a.count}</span></div>)}
            </div>
        </React.Fragment>
        )
}
